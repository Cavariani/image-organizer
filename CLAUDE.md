...
- **Relógio sintético por capítulo (marcos + interpolação)**: cada capítulo tem uma "hora de início" (`ch.timeStart`, min desde 0h = marco da foto 1) e qualquer foto pode fixar um marco (`im.clock`, min desde 0h, via 🕐 Fixar hora na barra de ações). `chapAnchors(ch)` junta os marcos por posição; `clockMinAt(ch,pos)` **interpola linearmente** entre marcos vizinhos (com 1 marco só, usa `ch.timeStep` como velocidade; sem marcos, EXIF). `photoClock(im,ch)` formata. Assim o relógio corre nos trechos densos (dia) e desacelera nos esparsos (golden hour) automaticamente. Editor do capítulo: 🕐 na sidebar (`openChapTimePop`, readout "1ª → última" ao vivo). `chapSpan` também usa isso.
