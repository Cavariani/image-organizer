...
- **Três modos** (cada um tem seu `render*`):
  ...
  - **Sequência** (`renderSequence`): grid com drag-and-drop, reordenar por índice (1-based). **1 clique num tile = menu de contexto no cursor** (`openTileMenu`: Abrir/Textos/Música/Stats/Tempo/Fixar hora/Mover/Rejeitar); **duplo clique = `openPhotoPreview`** (fullscreen com a foto + falas rodando como no Recall, via `#vnPreview`). Círculo de seleção (hover) faz multi-seleção → barra de ações embaixo (`actionBarHtml`, batch). Tile mostra hora calculada + indicadores (💬/🎵/🎬/stats). Popovers ancoram no cursor via âncora falsa `{getBoundingClientRect}`.
...
- **Visual novel** (`rcVN*` + `src/vn/`): cada fala é um doc rico **TipTap** (`{doc,fx,speed}`), normalizado de string legada por `normalizeFala`. `src/vn/render.ts` (`renderFala`) revela o texto por caractere/palavra com os efeitos (type/word/fade/blur) e estilo por trecho (negrito/cor/fonte/tamanho); `src/vn/tiptap.ts` monta o editor; `src/vn/fonts.ts` embute as fontes; `src/vn/effects.ts` tem os efeitos + velocidade global. Editor no modal `#textModal` (por-fala: toolbar + efeito + ▶ prévia); modos `'photo'` | `'cardBefore'` | `'cardAfter'`. No Recall, → completa/avança; `H` esconde. **Telas pretas entre fotos**: `im.cardBefore` e `im.cardAfter` (falas ricas). Na grade, cada uma vira um **bloco preto (`.cardBlock`)** antes/depois do tile. **Inserir**: hover na metade esquerda/direita do tile acende a borda (`.edgeL`/`.edgeR`); clicar na borda esquerda cria tela preta ANTES, direita DEPOIS (zonas = 20% de cada lado; o centro segue com menu/preview).
...
- **Relógio sintético por capítulo (marcos + linha do tempo contínua)**: UM conceito só — marcos = fotos com `im.clock` (hora de parede, min 0–1439). `chapAnchors` monta uma **linha do tempo contínua**: varrendo por posição, quando a hora "volta" (ex: 19:33→01:32) soma 24h internamente (madrugada = dia seguinte), então a interpolação é sempre crescente e o display wrap via `fmtMin`. `clockMins(ch)` interpola entre marcos e **extrapola nas pontas** (não congela depois do último marco); sem marcos, cai no EXIF. Editar (`openClockPin`, 🕐 no menu da foto) só guarda a HH:MM — a virada do dia é resolvida no cálculo, sem trava. `migrateClocks()` converte o modelo antigo. `photoClock`/`chapSpan`/tile usam `clockMins`. (Não há mais menu de horário por capítulo.)
- **HUD do Recall** (`rcHud`, `#rcHud`): capítulo (`ch.name`) + relógio (`photoClock`) no canto sup. direito, **persistente** (não some no idle, por isso fora de `.rcTop`). Sem hora mantém a última (`RC.hudTime`). O rodapé (`#rcTitle`/`#rcSub`) foi esvaziado; sobrou só a barra de progresso.
