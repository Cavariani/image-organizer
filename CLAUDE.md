...
- **Relógio sintético por capítulo** (`ch.timeStart` em min desde 0h + `ch.timeStep` min/foto): `photoClock(im,ch)` = `timeStart + (posição no capítulo)×timeStep`. Editado no 🕐 de cada capítulo na sidebar (`openChapTimePop`), com readout ao vivo "1ª → última". Sem `timeStart`, cai no EXIF (`im.taken`). Substituiu o antigo editor de hora por-foto. `chapSpan` também usa o sintético quando definido.
