...
- **Relógio sintético por capítulo (marcos, à prova de conflito)**: UM conceito só — marcos = fotos com `im.clock` (min desde 0h). `chapAnchors`/`clockMins(ch)` interpolam linearmente entre marcos por posição (fora do intervalo dos marcos, fixa na ponta); sem marcos, cai no EXIF. **Conflito é impossível por construção**: `clockBounds(ch,pos)` dá os limites (marco anterior/próximo) e a edição (`openClockPin` por foto; `openChapTimePop` = início/fim do dia na 1ª/última foto) clampa a hora entre os vizinhos. `photoClock`/`chapSpan`/tile usam `clockMins`. `migrateClocks()` converte o modelo antigo (`ch.timeStart/timeEnd/timeStep`) em marcos. Editor: 🕐 na sidebar (capítulo) e 🕐 no menu da foto.
